...
/* ---------- Helpers to derive user state ---------- */
function getUserIdFromToken() {
  const token = localStorage.getItem('token');
  if (!token) return null;
  try { return JSON.parse(atob(token.split('.')[1])).userId; } catch (_) { return null; }
}

// Derive whether the user has signed up before (first-time sign in → unlock)
function isNewUserUnlocked() {
  // If we have ever seen a successful sign-in, set a flag in localStorage
  // signIn.js already stores token; we consider presence of token as signed-in.
  const token = localStorage.getItem('token');
...
async function fetchLevelXP() {
  ...
  try {
    ...
    const data = await res.json();
    return { level: Number(data.level || 1), xp: Number(data.xp || 0), nextLevelXP: Number(data.nextLevelXP || 100) };
  } catch (_) {
    return { level: 1, xp: 0, nextLevelXP: 100 };
  }
}

/* ---------- Two achievements only ---------- */
const achievements = [
  {
    id: 1,
    name: 'New User',
  ...
  },
  {
    ...
    icon: '🔥',
    // condition checked asynchronously in render
    condition: async () => (await fetchCurrentStreak()) >= 5,
    colors: ['#3b82f6', '#06b6d4']
  }
];

/* ---------- One badge: Complete Level 1 ---------- */
const badges = [
  {
    id: 'lvl1-complete',
    ...
    condition: async () => {
      ...
      // Unlock if user is already level >= 2 OR has filled XP to threshold
...
async function renderAchievements() {
  ...
  for (const a of achievements) {
    const unlocked = typeof a.condition === 'function' ? await a.condition() : false;
...
async function renderBadges() {
  ...
  for (const b of badges) {
    const unlocked = typeof b.condition === 'function' ? await b.condition() : false;
    const card = document.createElement('div');
