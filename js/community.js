// Community JavaScript for Zenity
class CommunityApp {
    constructor() {
        this.socket = null;
    ...
    async loadChannels() {
        try {
            const response = await fetch('http://localhost:8001/api/community/channels');
            const data = await response.json();
    ...
    async selectChannel(channel) {
        ...
        document.querySelector(`[data-channel-id="${channel._id}"]`).classList.add('active');
    ...
    async loadMessages(channelId) {
        try {
            const response = await fetch(`http://localhost:8001/api/community/channels/${channelId}/messages`);
            const data = await response.json();
    ...
    async sendMessage() {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`http://localhost:8001/api/community/channels/${this.currentChannel._id}/messages`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
