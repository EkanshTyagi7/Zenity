...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(
    `http://localhost:8001/api/log/get?userId=${userId}&date=${currentDate}`
  )
    .then((res) => res.json())
    .then((data) => {
      if (data && data._id) {
    ...
    .then((data) => {
      ...
      try {
        ...
        localStorage.setItem(
          "lastSleepData",
          JSON.stringify({ date: todayLog.date, sleep: todayLog.sleep })
        );
        localStorage.setItem(
          "lastMentalData",
          JSON.stringify({
...
async function updateDashboardStreaks() {
  try {
    ...
    const response = await fetch(
      `http://localhost:8001/api/log/streaks/${userId}`,
      {
...
function populateLogForm() {
  ...
  moodOptions.forEach((option) => {
    ...
    btn.className =
      "mood-option" + (todayLog.mood === option.value ? " selected" : "");
  ...
  renderScale(
    ...
    "Social Interaction",
    "users",
    todayLog.socialInteraction,
    1,
    5,
    ["Isolated", "Very Social"],
    (val) => {
      todayLog.socialInteraction = val;
      populateLogForm();
    }
  );
  renderScale(
  ...
  renderScale(
    ...
    "Productivity",
    "target",
    todayLog.productivity,
    1,
    5,
    ["Unproductive", "Highly Productive"],
    (val) => {
      todayLog.productivity = val;
      populateLogForm();
    }
  );
  renderScale(
    ...
    "brain",
    todayLog.mindfulness,
    1,
    5,
    ["None", "Extensive"],
    (val) => {
      todayLog.mindfulness = val;
      populateLogForm();
    }
  );
...
function updateSymptomsUI() {
  ...
  symptomOptions.forEach((symptom) => {
    ...
    btn.className =
      "symptom-button" +
      (todayLog.symptoms.includes(symptom) ? " selected" : "");
...
function updateActivitiesUI() {
  ...
  activityOptions.forEach((activity) => {
    ...
    btn.className =
      "activity-button" +
      (todayLog.activities.includes(activity.name) ? " selected" : "");
...
function attachEventListeners() {
  ...
  document
    ...
    .addEventListener("click", handleLogSubmit);
}
