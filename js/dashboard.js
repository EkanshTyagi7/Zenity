  ...
  async function loadUserLevelXP() {
    try {
      ...
      if (data.success) {
        ...
        if (xpBar) {
          const percent =
            data.nextLevelXP > 0 ? (data.xp / data.nextLevelXP) * 100 : 100;
...
function initializeSleepChartCard() {
  ...
  (async () => {
    try {
      ...
      const log = data.log;
      const hours =
        typeof log.sleep.hours === "number" ? log.sleep.hours : null;
      const quality =
        typeof log.sleep.quality === "number" ? log.sleep.quality : null;
        ...
        tooltip: {
          callbacks: {
            label: (ctx) => {
              ...
              if (ctx.dataIndex === 1)
                return `Quality: ${qualityDisplay} (${quality}/5)`;
            },
          },
        },
      },
  ...
  (async () => {
    try {
      ...
      if (dateLabel && log.date) {
        ...
        dateLabel.textContent = isNaN(d.getTime())
          ? log.date
...
function initializeReadSection() {
  ...
  const pool = [
    ...
    {
      title: "Sleep Hygiene: Small Habits, Big Impact",
      source: "Zenity Guides",
    },
...
function renderRead(picks) {
  ...
  picks.forEach((p, i) => {
    ...
    li.innerHTML = `
      <div class="read-thumb" style="background: ${
        ["#c8b8ff", "#b7a8f7", "#8d71eb"][i % 3]
      }"></div>
      <div class="read-meta">
...
async function initializeStreakCalendar() {
  ...
  if (userId) {
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      promises.push(
        fetch(
          `http://localhost:8001/api/log/get?userId=${userId}&date=${dateStr}`
        )
          .then((r) => r.json())
          .then((data) => {
            if (data && data._id) dayHasLog.add(d);
          })
          .catch(() => {})
      );
    }
